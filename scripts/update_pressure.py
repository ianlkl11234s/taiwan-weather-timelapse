...
Usage:
    ...
    python scripts/update_pressure.py

    # Limit to recent days
    python scripts/update_pressure.py --days 7

    # Specify date range
    python scripts/update_pressure.py --start-date 2025-01-10 --end-date 2025-01-15
"""
...
OUTPUT_FILE = PUBLIC_DIR / 'pressure_timelapse_data.json'
...
def interpolate_pressure(stations: List[Dict], geo_info: Dict) -> Tuple[List[List], Dict]:
    ...
    # Clip values to valid pressure range (900-1100 hPa)
    # Cubic interpolation can produce values outside the input range at edges
    grid_pressure = np.clip(grid_pressure, 900, 1100)

    # Apply land mask from temperature data
    # This ensures pressure is only shown where temperature data exists (land areas)
    land_mask = get_land_mask()
    if land_mask is not None:
        # Set ocean areas to NaN
        grid_pressure[~land_mask] = np.nan

    # Calculate statistics (excluding NaN and clipped edge values)
    valid_grid = grid_pressure[~np.isnan(grid_pressure)]
    # Exclude values that were clipped to exactly 900 or 1100 (edge artifacts)
    inner_values = valid_grid[(valid_grid > 900.1) & (valid_grid < 1099.9)]
...
def download_and_interpolate_pressure(
    ...
    """Download weather data and interpolate pressure"""
...
def main():
    parser = argparse.ArgumentParser(
        description='Update Taiwan Pressure Timelapse data'
    )
    parser.add_argument(
        '--env-file',
        type=Path,
    ...
    print("=" * 60)
    print("Taiwan Pressure Timelapse - Data Update")
    print("=" * 60)
    ...
    # Download and interpolate
    frames = download_and_interpolate_pressure(
        reader,
    ...
    if not frames:
        print("ERROR: No pressure data available")
        sys.exit(1)
